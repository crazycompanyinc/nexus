...
class TestResponseTimeHeader:
    ...
    def test_timing_changes_with_workload(self, client):
        """Two identical requests should both have timing headers."""
        r1 = client.get("/health")
        r2 = client.get("/health")
        assert "X-Response-Time" in r1.headers
        assert "X-Response-Time" in r2.headers
...
class TestShutdown:
    """Verify shutdown handler is registered and doesn't error."""

    def test_app_has_shutdown_handler(self):
        """create_app should register a shutdown event handler."""
        app = create_app()
        # FastAPI stores handlers in _startup_handlers and _shutdown_handlers
        # For on_event("shutdown"), check router has the handler
        shutdown_handlers = [
            fn for fn in getattr(app, "router", app).__dict__.get("on_shutdown", [])
        ]
        # At minimum, the app should have a state with store
